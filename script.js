...
function parseAgeInput(input) {
    ...
    for (let pattern of datePatterns) {
        ...
        if (match) {
            let day, month, year;
            if (pattern.source.startsWith('^(')) { // DD/MM/YYYY
...
// ==========================================
// 3. DATOS DE RESPALDO Y VARIABLES GLOBALES
// ==========================================
const FALLBACK_DB = {
"dogs": [
{
...
if (d.perfil.fecha_nacimiento) {
    if (isBirthdayToday(d.perfil.fecha_nacimiento)) {
        ...
        birthdayBadge = '<span class="birthday-badge">🎈 Cumpleaños próximo</span>';
    }
}
...
try {
const ageInput = document.getElementById('new-dog-age').value.trim();
...
// Feedback en tiempo real en formulario de creación
document.getElementById('new-dog-age')?.addEventListener('input', (e) => {
const status = document.getElementById('create-age-status');
if (!status) return;
const valid = parseAgeInput(e.target.value) !== null;
status.textContent = valid ? '✅' : '❓';
status.style.color = valid ? 'var(--success)' : 'var(--warning)';
});
...
<input type="text" name="edad_input" value="${p.edad_input || p.edad || ''}" 
       placeholder="Ej: 3 años, 5 meses, 20/07/2025" id="edit-edad-input">
<span id="edit-age-status" style="font-size:1.2rem;"></span>
</div>
...
const valid = parseAgeInput(ageInputField.value) !== null;
ageStatus.textContent = valid ? '✅' : '❓';
ageStatus.style.color = valid ? 'var(--success)' : 'var(--warning)';
});
...
const parsedDate = parseAgeInput(rawInput);
updatedPerfil.edad_input = rawInput;
if (parsedDate) {
updatedPerfil.fecha_nacimiento = parsedDate;
updatedPerfil.edad = calculateExactAge(parsedDate);
} else {
updatedPerfil.fecha_nacimiento = null;
updatedPerfil.edad = rawInput;
}
} else {
